...
job = client.beta.split.create(
    ...
    configuration={
        "version": "latest",  # or a dated release like "2026-09-18", or a preview like "jev-preview" or "turbo-preview"
        "categories": [
            {
                "name": "invoice",
...
func main() {
	...
	job, err := client.Beta.Split.New(ctx, llamacloud.BetaSplitNewParams{
		...
		Configuration: llamacloud.BetaSplitNewParamsConfiguration{
			Version: llamacloud.String("latest"), // or a dated release like "2026-09-18", or a preview like "jev-preview" or "turbo-preview"
...
```
    ...
    **Versions.** `configuration.version` is optional and follows the same taxonomy as Parse and Extract. `latest` (the default) resolves to the current dated release; today that is `2026-09-18`, which runs the standard per-page generative classifier over parsed text. Pin the dated string in production so upgrades are opt-in. Preview names are selectable by name only and are never picked up by `latest`; a job stays on the preview version you named until you switch off it. Two previews are available:

    - `jev-preview` — batched classification backend that answers the classifier's fixed questions in one request per page batch instead of one generative call per page. Aimed at lower latency and cost while still reading parsed text.
    - `turbo-preview` — classifies straight from rendered page images and runs no parse step, so it skips the parse stage entirely. Useful when you want faster splits and don't need parsed text alongside the classification.
